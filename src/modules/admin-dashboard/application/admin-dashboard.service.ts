...
export class AdminDashboardService {
  ...
  async getSummary(userId: string): Promise<AdminDashboardSummary> {
    const [
      ...
      // Courses and Seminars are separate ManagementScreen cards, each opening
      // a type-scoped AdminVetCoursesScreen — so each has its own count + seen cursor.
      this.deps.vetCourses.listForModeration({ ...WINDOW, type: 'COURSE' }),
      this.deps.vetCourses.listForModeration({ ...WINDOW, type: 'SEMINAR' }),
