...
import {
  ...
  type SheepDailyRecord,
  type SheepDailyRecordRow,
  type UpdateSheepDailyRecordInput,
...
export class SheepDailyRecordRepository {
  ...
  async findByBatchAndDate(
    ...
    const row = await this.conn(trx)<SheepDailyRecordRow>(TABLE)
      .where({ sheep_batch_id: batchId, record_date: recordDate })
      .first();
    return row ? rowToSheepDailyRecord(row) : null;
  }

  async create(
    data: CreateSheepDailyRecordInput & {
      sheepBatchId: string;
    ...
    return rowToSheepDailyRecord(row);
  }
  ...
  async listInRange(batchId: string, from: string, to: string): Promise<SheepDailyRecord[]> {
    const rows: SheepDailyRecordRow[] = await this.db<SheepDailyRecordRow>(TABLE)
      .where('sheep_batch_id', batchId)
      .andWhere('record_date', '>=', from)
      .andWhere('record_date', '<=', to)
      .orderBy('record_date', 'asc');
    return rows.map(rowToSheepDailyRecord);
  }

  /** Whole-batch aggregates for the batch-summary card. */
  async aggregateForBatch(batchId: string): Promise<SheepDailyRecordAggregates> {
  ...
  async latestWeightForBatch(batchId: string): Promise<string | null> {
    const row = (await this.db<SheepDailyRecordRow>(TABLE)
      .where('sheep_batch_id', batchId)
