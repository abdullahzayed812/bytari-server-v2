...
import {
  ...
  type SheepCase,
  type SheepCaseSummary,
  type UpdateSheepCaseInput,
} from '../domain/sheep-ops.types.js';
import type { SheepCaseRepository } from '../infrastructure/sheep-case.repository.js';
...
/** "الحالات الفردية" — individual sick sheep tracked to recovery / death. */
export class SheepCaseService {
  private readonly log: Logger;

  constructor(
    private readonly db: Knex,
    private readonly cases: SheepCaseRepository,
    ...
    this.log = logger.child({ component: 'sheep-case-service' });
  }
  ...
  ): Promise<{ items: SheepCase[]; total: number }> {
    await this.assertBatch(organizationId, batchId);
    const { rows, total } = await this.cases.listForBatch(batchId, filter);
    const items = await Promise.all(
      rows.map(async (r) => rowToSheepCase(r, await this.resolveImageUrl(r.image_key))),
    );
    return { items, total };
  }

  async summary(organizationId: string, batchId: string): Promise<SheepCaseSummary> {
    await this.assertBatch(organizationId, batchId);
  ...
  async get(organizationId: string, batchId: string, caseId: string): Promise<SheepCase> {
    await this.assertBatch(organizationId, batchId);
    const row = await this.cases.findRowByIdForBatch(caseId, batchId);
    if (!row) throw new NotFoundError('Case not found');
    return rowToSheepCase(row, await this.resolveImageUrl(row.image_key));
  }

  async create(
    organizationId: string,
    batchId: string,
    input: CreateSheepCaseInput,
    actor: FarmActor,
  ): Promise<SheepCase> {
    await this.assertBatch(organizationId, batchId);
    const row = await this.db.transaction(async (tx) => {
      ...
      const created = await this.cases.create(
        { ...input, sheepBatchId: batchId, organizationId, caseNumber, createdByUserId: actor.actorUserId },
        tx,
      );
      await this.audit.record(
        {
          action: SheepOpsAuditAction.SHEEP_CASE_CREATED,
          entityType: SheepOpsAuditEntity.SHEEP_CASE,
          entityId: created.id,
          actorUserId: actor.actorUserId,
          metadata: { organizationId, sheepBatchId: batchId, caseNumber },
    ...
    this.events.publish('sheep.case.created', { organizationId, sheepBatchId: batchId, caseId: row.id });
    return rowToSheepCase(row, null);
  }
  ...
  async update(
    ...
    const row = await this.db.transaction(async (tx) => {
      ...
      await this.audit.record(
        {
          action: SheepOpsAuditAction.SHEEP_CASE_UPDATED,
          entityType: SheepOpsAuditEntity.SHEEP_CASE,
          entityId: caseId,
          actorUserId: actor.actorUserId,
  ...
  async delete(organizationId: string, batchId: string, caseId: string, actor: FarmActor): Promise<void> {
    ...
    await this.db.transaction(async (tx) => {
      ...
      await this.audit.record(
        {
          action: SheepOpsAuditAction.SHEEP_CASE_DELETED,
          entityType: SheepOpsAuditEntity.SHEEP_CASE,
          entityId: caseId,
          actorUserId: actor.actorUserId,
      ...
      await this.audit.record(
        {
          ...
          entityId: caseId,
          actorUserId: actor.actorUserId,
          metadata: { organizationId, sheepBatchId: batchId, imageUpdated: true },
          context: actor.context,
        },
        tx,
      );
