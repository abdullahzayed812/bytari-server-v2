...
import { VeterinarianStoreController } from './veterinarian-store.controller.js';
import { VeterinarianStoreAdminController } from './veterinarian-store-admin.controller.js';
import {
...
/**
 * Consumer Veterinarian Store, mounted at `/veterinarian-store`. Every route is
 ...
 */
export function createVeterinarianStoreRouter(c: Container): Router {
  const ctrl = new VeterinarianStoreController(
    c.vetStoreCatalogService,
    c.vetStoreCartService,
    c.vetStoreOrderService,
  );
...
/**
 * Admin / supervisor management, mounted at `/admin/veterinarian-store`. Guarded by
 * `authorize('veterinarian_store.<area>.manage')` — the ADMIN override or an ACTIVE
 * `VETERINARIAN_STORE` system-supervisor assignment satisfies it. The permission
 * check is server-side, never UI-only.
 */
export function createAdminVeterinarianStoreRouter(c: Container): Router {
  const ctrl = new VeterinarianStoreAdminController(c.vetStoreAdminService, c.vetStoreOrderService);
  const { authorize } = c.authorization;
  const manageProducts = authorize('veterinarian_store.product.manage');
  const manageCategories = authorize('veterinarian_store.category.manage');
  const manageOrders = authorize('veterinarian_store.order.manage');
