...
const CARTS = 'pet_owner_store_carts';
const ITEMS = 'pet_owner_store_cart_items';

export interface PetStoreCartRow {
  id: string;
  user_id: string;
  created_at: Date;
  updated_at: Date;
...
export interface PetStoreCartItemRow {
  id: string;
  cart_id: string;
  product_id: string;
...
export class PetStoreCartRepository {
  ...
  async findCartByUser(userId: string, trx?: Knex.Transaction): Promise<PetStoreCartRow | null> {
    const row = await this.conn(trx)<PetStoreCartRow>(CARTS).where({ user_id: userId }).first();
    return row ?? null;
  }

  async getOrCreateCart(userId: string, trx: Knex.Transaction): Promise<PetStoreCartRow> {
    const existing = await trx<PetStoreCartRow>(CARTS).where({ user_id: userId }).first();
    if (existing) return existing;
    const [row] = (await trx(CARTS)
      .insert({ user_id: userId })
      .onConflict('user_id')
      .merge({ updated_at: new Date() })
      .returning('*')) as PetStoreCartRow[];
    if (!row) throw new Error('pet store cart upsert did not return a row');
    return row;
  }
  ...
  async listItemsWithProduct(
    ...
    const rows = await this.conn(trx)(`${ITEMS} as i`)
      .join('pet_owner_store_products as p', 'p.id', 'i.product_id')
  ...
  async findItem(
    ...
    productId: string,
    trx?: Knex.Transaction,
  ): Promise<PetStoreCartItemRow | null> {
    const row = await this.conn(trx)<PetStoreCartItemRow>(ITEMS)
