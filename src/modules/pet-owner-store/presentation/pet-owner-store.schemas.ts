...
import {
  ...
  PET_STORE_CATEGORY_STATUSES,
  PET_STORE_MAX_IMAGE_BYTES,
  PET_STORE_MAX_ITEM_QUANTITY,
  PET_STORE_ORDER_STATUSES,
  PET_STORE_PAYMENT_METHODS,
  PET_STORE_PRODUCT_SORTS,
  PET_STORE_PRODUCT_STATUSES,
...
const quantitySchema = z.coerce.number().int().min(1).max(PET_STORE_MAX_ITEM_QUANTITY);
...
export const listProductsQuerySchema = paginationQuerySchema.extend({
  ...
  sort: z.enum(PET_STORE_PRODUCT_SORTS).optional(),
...
export const checkoutBodySchema = z.object({
  paymentMethod: z.enum(PET_STORE_PAYMENT_METHODS),
...
export const listOrdersQuerySchema = paginationQuerySchema.extend({
  status: z.enum(PET_STORE_ORDER_STATUSES).optional(),
});
...
export const adminListProductsQuerySchema = listProductsQuerySchema.extend({
  status: z.enum(PET_STORE_PRODUCT_STATUSES).optional(),
});
...
export const createCategoryBodySchema = z.object({
  ...
  status: z.enum(PET_STORE_CATEGORY_STATUSES).optional(),
});
...
export const imageUploadUrlBodySchema = z.object({
  ...
  mimeType: z.enum(PET_STORE_ALLOWED_IMAGE_MIME),
  size: z.coerce.number().int().positive().max(PET_STORE_MAX_IMAGE_BYTES),
});
...
export const updateOrderStatusBodySchema = z.object({
  status: z.enum(PET_STORE_ORDER_STATUSES),
});
