...
const schemas: Obj = {
  ...
  PublicVetCourse: {
    ...
    properties: {
      ...
      capacity: { type: 'integer', nullable: true },
      registrationCount: { type: 'integer' },
      remainingSeats: { type: 'integer', nullable: true, description: 'capacity - registrationCount; null = unlimited' },
      isRegistered: { type: 'boolean', description: 'The caller already holds a registration' },
      registrationState: {
        type: 'string',
        enum: ['OPEN', 'REGISTERED', 'FULL', 'CLOSED'],
        description: 'Caller-relative: REGISTERED wins, then FULL (no seats), then CLOSED (past cutoff)',
      },
      price: { ...money, nullable: true },
      registrationDeadline: { type: 'string', format: 'date', nullable: true },
      topics: { type: 'array', items: { type: 'string' } },
