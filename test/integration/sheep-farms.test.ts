...
async function setup() {
  ...
  const farm = await createFarm(app, owner.accessToken, admin.accessToken, { name: 'Sheep Co' });
...
const batchPath = (orgId: string, id?: string): string =>
  `/api/v1/organizations/${orgId}/sheep/batches${id ? `/${id}` : ''}`;

describe('POST /organizations/sheep-farms — Add Sheep Farm', () => {
  it('creates a SHEEP-species FARM org (PENDING) with the profile fields round-tripped', async () => {
    const admin = await registerAdmin(app);
    const owner = await registerApprovedVet(app);
    ...
    // the generic (species-agnostic) farm/profile endpoint round-trips the
    // sheep-specific fields written at creation — no new profile route needed.
  ...
  it('a poultry farm created via /organizations/farms is auto-tagged farmSpecies=POULTRY', async () => {
    const admin = await registerAdmin(app);
    const owner = await registerApprovedVet(app);
    const create = await request(app)
    ...
    const profile = await request(app)
      .get(`/api/v1/organizations/${orgId}/farm/profile`)
      .set(bearer(owner.accessToken));
    expect(profile.body.data.farmSpecies).toBe('POULTRY');
  });
...
describe('sheep batches — CRUD', () => {
  ...
  it('closing a batch stamps closed_at and blocks further content edits (409)', async () => {
    ...
    expect(edit.body.error.code).toBe('SHEEP_BATCH_NOT_ACTIVE');
...
describe('sheep batches — authorization', () => {
  ...
  it('respects a farm SUPERVISOR’s explicitly-assigned permissions', async () => {
    ...
    await assignOrganizationSupervisor(app, owner.accessToken, farm.id, {
      ...
      permissions: ['farm.sheep_batch.read'],
    });
  ...
  it('lets an ADMIN operate on any farm’s sheep batches', async () => {
    const { admin, farm } = await setup();
    const res = await request(app)
      .post(batchPath(farm.id))
      .set(bearer(admin.accessToken))
      .send({ name: 'Admin batch', headCount: 50, arrivalDate: '2026-01-01' });
    expect(res.status).toBe(201);
  });

  it('rejects sheep-batch operations on a non-FARM organization with 400', async () => {
    const admin = await registerAdmin(app);
    const owner = await registerApprovedVet(app);
...
describe('sheep batches — cross-farm isolation (IDOR)', () => {
  ...
  it('the database composite FK refuses a batch that points at a non-FARM organization', async () => {
    ...
    await expect(
      getTestDb()('sheep_batches').insert({
